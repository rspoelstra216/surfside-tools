...
function surfside_tools_sanitize_ministries($ministries) {
    ...
    foreach ($ministries as $index => $ministry) {
        ...
        $audiences = isset($ministry['audiences']) && is_array($ministry['audiences'])
            ? array_map('sanitize_key', $ministry['audiences']) : array('adults');
        $audiences = array_values(array_unique(array_intersect($audiences, $allowed_audiences)));
        if (empty($audiences)) $audiences = array('adults');

        // Existing records predate this field and should remain visible as cards
        // until staff explicitly unchecks Featured Ministry and saves them.
...
function surfside_tools_get_ministries() {
    ...
    foreach ($legacy as &$ministry) {
        ...
        if (is_array($ministry) && !array_key_exists('featured', $ministry)) $ministry['featured'] = true;
    }
    unset($ministry);
