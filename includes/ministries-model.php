...
function surfside_tools_ministry_audience_choices() {
    return array('kids'=>'Kids','youth'=>'Youth','adults'=>'Adults','all_ages'=>'All Ages');
}
...
function surfside_tools_sanitize_ministries($ministries) {
    ...
    foreach ($ministries as $index => $ministry) {
        ...
        if ($key === '') $key = 'ministry-' . substr(md5(wp_json_encode(array($name, $index))), 0, 12);
        $audiences = isset($ministry['audiences']) && is_array($ministry['audiences']) ? array_map('sanitize_key',$ministry['audiences']) : array('adults');
...
function surfside_tools_get_ministries() {
    ...
    if(is_array($stored)&&!empty($stored)) return surfside_tools_sanitize_ministries($stored);
    $information=function_exists('surfside_tools_get_site_information')?surfside_tools_get_site_information():array();
    $legacy=isset($information['adult_ministries'])&&is_array($information['adult_ministries'])?$information['adult_ministries']:array();
    foreach($legacy as &$ministry){if(is_array($ministry)&&empty($ministry['audiences']))$ministry['audiences']=array('adults');if(is_array($ministry)&&!array_key_exists('featured',$ministry))$ministry['featured']=true;}unset($ministry);
...
function surfside_tools_resolve_ministry_contact($ministry) {
    ...
    $source='ministry';
    if($email===''){ $email=surfside_tools_get_ministry_default_email(); $source='default'; }
    return array(
...
function surfside_tools_ministry_audience_labels($ministry) {
    ...
    $audiences=isset($ministry['audiences'])&&is_array($ministry['audiences'])?$ministry['audiences']:array('adults');
    $labels=array();foreach($audiences as $audience)if(isset($choices[$audience]))$labels[]=$choices[$audience];return $labels;
}

function surfside_tools_register_ministries_mobile_route() {
    register_rest_route('surfside/v1','/ministries',array('methods'=>WP_REST_Server::READABLE,'callback'=>'surfside_tools_mobile_api_ministries','permission_callback'=>'__return_true'));
}
...
function surfside_tools_mobile_api_ministries() {
    ...
    foreach((array)surfside_tools_get_ministries() as $ministry){
        $ministries[]=array(
            'key'=>(string)($ministry['key']??''),'icon'=>(string)($ministry['icon']??''),'name'=>(string)($ministry['name']??''),
            'schedule'=>(string)($ministry['schedule']??''),'location'=>(string)($ministry['location']??''),'description'=>(string)($ministry['description']??''),
            'audiences'=>array_values((array)($ministry['audiences']??array())),'audience_labels'=>surfside_tools_ministry_audience_labels($ministry),'featured'=>!empty($ministry['featured']),
            'contact'=>surfside_tools_resolve_ministry_contact($ministry),
        );
    }
...
<style>.surfside-all-ministries__intro{text-align:center}.surfside-all-ministries__filters{justify-content:center}</style>
<?php }
