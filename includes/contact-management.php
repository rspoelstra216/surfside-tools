...
function surfside_tools_staff_contact_management_shortcode(){
    ...
    if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['surfside_contact_nonce'])&&wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['surfside_contact_nonce'])),'surfside_contact_save')){
        ...
        update_option('surfside_tools_contact_settings',array('recipients'=>$recipients)); $saved=true;
    }
    ...
    <div class="surfside-staff-shell">
      ...
      <?php if($saved):?><div style="padding:14px 18px;margin-bottom:20px;border-radius:10px;background:#eaf7ef;color:#126b36;font-weight:700">Contact routing saved.</div><?php endif; ?>
      <form method="post"><?php wp_nonce_field('surfside_contact_save','surfside_contact_nonce'); ?>
        ...
        </div></section><button type="submit" class="surfside-staff-button" style="width:auto;min-width:190px;border:0;cursor:pointer">Save Contact Routing</button>
      </form>
