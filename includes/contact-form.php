...
function surfside_tools_contact_form_handler(){
    ...
    if($count>=5){set_transient('surfside_contact_values_'.wp_get_session_token(),$data,MINUTE_IN_SECONDS);wp_safe_redirect(add_query_arg('surfside_error','rate',$return_url).'#Contact');exit;}
    $turnstile=surfside_tools_verify_turnstile(sanitize_text_field(wp_unslash($_POST['cf-turnstile-response']??'')));
    if(is_wp_error($turnstile)){set_transient('surfside_contact_values_'.wp_get_session_token(),$data,MINUTE_IN_SECONDS);wp_safe_redirect(add_query_arg('surfside_error','turnstile',$return_url).'#Contact');exit;}
    $sent=surfside_tools_contact_send($data);
    if(is_wp_error($sent)){set_transient('surfside_contact_error_'.wp_get_session_token(),$sent->get_error_message(),MINUTE_IN_SECONDS);set_transient('surfside_contact_values_'.wp_get_session_token(),$data,MINUTE_IN_SECONDS);wp_safe_redirect(add_query_arg('surfside_error','send',$return_url).'#Contact');exit;}
    set_transient($rate_key,$count+1,HOUR_IN_SECONDS); delete_transient('surfside_contact_values_'.wp_get_session_token()); wp_safe_redirect(add_query_arg('surfside_contact','sent',$return_url).'#Contact');exit;
}
...
function surfside_tools_contact_form_shortcode(){
    ...
    $defaults=array('category'=>'general','name'=>'','email'=>'','phone'=>'','message'=>'','preferred_contact'=>'email','prayer_privacy'=>'prayer-team','prayer_name_display'=>'named','prayer_duration'=>'14');
    $stored_values=get_transient('surfside_contact_values_'.wp_get_session_token());
    $values=is_array($stored_values)?wp_parse_args($stored_values,$defaults):$defaults;
    if(is_array($stored_values))delete_transient('surfside_contact_values_'.wp_get_session_token());
    ...
    <section class="surfside-contact-form" aria-label="Contact Surfside" id="Contact">
      ...
      <form method="post" action="<?php echo esc_url(get_permalink()); ?>" class="surfside-contact-form__form"><?php wp_nonce_field('surfside_contact_form','surfside_contact_form_nonce');?><input type="text" name="website" tabindex="-1" autocomplete="off" class="surfside-contact-form__trap" aria-hidden="true">
        ...
        <label>Name *<input required name="name" autocomplete="name" value="<?php echo esc_attr($values['name']);?>"></label>
        <div class="surfside-contact-form__contact-heading">Email or Phone * <span>At least one is required.</span></div>
        <div class="surfside-contact-form__row"><label>Email<input type="email" name="email" autocomplete="email" value="<?php echo esc_attr($values['email']);?>"></label><label>Phone<input type="tel" name="phone" autocomplete="tel" value="<?php echo esc_attr($values['phone']);?>"></label></div>
        ...
        <div id="surfside-contact-prayer">
          ...
          <div id="surfside-contact-prayer-public" class="surfside-contact-prayer-public">
            ...
            <fieldset><legend>How should your name appear?</legend><label><input type="radio" name="prayer_name_display" value="named" <?php checked($values['prayer_name_display'],'named');?>> Show my name</label><label><input type="radio" name="prayer_name_display" value="anonymous" <?php checked($values['prayer_name_display'],'anonymous');?>> Share anonymously</label></fieldset>
            <label>How long would you like this prayer request to remain active?<select name="prayer_duration"><option value="7" <?php selected($values['prayer_duration'],'7');?>>7 days</option><option value="14" <?php selected($values['prayer_duration'],'14');?>>14 days</option><option value="30" <?php selected($values['prayer_duration'],'30');?>>30 days</option></select></label>
          </div>
        </div>
