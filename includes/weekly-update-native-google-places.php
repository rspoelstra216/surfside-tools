...
function surfside_tools_weekly_update_native_google_places_assets() {
    ...
    (function () {
        ...
        function initializeTarget(target) {
            if (!target || !target.matches || !target.matches('.surfside-calendar-required-venue')) return;
            if (initializeInput(target)) return;
            let attempts = 0;
            const retry = window.setInterval(function () {
                attempts++;
                if (initializeInput(target) || attempts >= 80) window.clearInterval(retry);
            }, 250);
        }
        document.addEventListener('focusin', function (event) { initializeTarget(event.target); });
        document.addEventListener('pointerdown', function (event) { initializeTarget(event.target); });
        document.addEventListener('input', function (event) { initializeTarget(event.target); }, true);
