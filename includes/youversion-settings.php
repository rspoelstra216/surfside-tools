<?php
/**
 * Front-end Integrations UI for YouVersion.
 */
...
function surfside_tools_youversion_settings_panel() {
    ...
    ob_start(); ?>
    <details id="surfside-youversion" class="surfside-front-settings-card surfside-integration-card surfside-youversion-settings-card">
        <summary><span>YouVersion</span><span class="surfside-youversion-status surfside-integration-status <?php echo $configured ? 'is-configured' : ''; ?>"><?php echo $configured ? 'Configured' : 'Not configured'; ?></span></summary>
        <div class="surfside-integration-body">
            ...
            <?php if (is_array($test)) : ?>
                ...
                <?php if (!empty($test['versions']) && is_array($test['versions'])) : ?>
                    <details class="surfside-youversion-diagnostic surfside-youversion-versions"><summary>Available versions <span><?php echo esc_html(number_format_i18n(count($test['versions']))); ?></span></summary><div class="surfside-youversion-diagnostic-body"><ul><?php foreach ($test['versions'] as $version) : ?><li><?php echo esc_html(trim(($version['abbreviation'] ?? '') . (($version['abbreviation'] ?? '') && ($version['title'] ?? '') ? ' — ' : '') . ($version['title'] ?? ''))); ?></li><?php endforeach; ?></ul></div></details>
                <?php endif; ?>
                <?php if (!empty($test['proof']) && is_array($test['proof'])) : $proof = $test['proof']; ?>
                    <details class="surfside-youversion-diagnostic surfside-youversion-proof"><summary>Verse proof <span><?php echo esc_html(($proof['reference'] ?? 'John 3:16') . ' · ' . ($proof['version'] ?? 'BSB')); ?></span></summary><div class="surfside-youversion-diagnostic-body"><p><?php echo esc_html($proof['content'] ?? ''); ?></p><?php if (!empty($proof['copyright'])) : ?><small><?php echo esc_html($proof['copyright']); ?></small><?php endif; ?><?php if (!empty($proof['deep_link'])) : ?><a href="<?php echo esc_url($proof['deep_link']); ?>" target="_blank" rel="noopener">Open version in YouVersion ↗</a><?php endif; ?></div></details>
                <?php endif; ?>
            <?php endif; ?>
    ...
    <style>#surfside-youversion{scroll-margin-top:24px}.surfside-youversion-notice{margin:12px 0 0!important;padding:9px 11px!important;font-size:.92rem}.surfside-youversion-form{margin-top:14px}.surfside-youversion-row{display:grid;grid-template-columns:minmax(220px,1fr) auto auto auto;gap:8px;align-items:center}.surfside-youversion-row input{width:100%;box-sizing:border-box;padding:9px 11px;border:1px solid #9aa9b8;border-radius:7px;font:inherit}.surfside-youversion-button{width:auto!important;min-width:0!important;margin:0!important;padding:9px 14px!important;white-space:nowrap}.surfside-youversion-meta{display:flex;justify-content:space-between;gap:16px;flex-wrap:wrap;margin-top:7px;color:#65758a;font-size:.82rem}.surfside-youversion-meta label{white-space:nowrap}.surfside-youversion-diagnostic{margin-top:8px;border:1px solid #d8e0e7;border-radius:8px;background:#f8fafc;overflow:hidden;font-size:.9rem}.surfside-youversion-diagnostic summary{display:flex;align-items:center;gap:8px;padding:9px 11px;cursor:pointer;font-weight:650;color:#334155;list-style:none}.surfside-youversion-diagnostic summary::-webkit-details-marker{display:none}.surfside-youversion-diagnostic summary:before{content:'›';display:inline-block;font-size:1.15rem;line-height:1;color:#64748b;transition:transform .15s ease}.surfside-youversion-diagnostic[open] summary:before{transform:rotate(90deg)}.surfside-youversion-diagnostic summary span{margin-left:auto;font-size:.8rem;font-weight:600;color:#64748b}.surfside-youversion-diagnostic[open] summary{border-bottom:1px solid #e2e8f0}.surfside-youversion-diagnostic-body{padding:10px 12px;background:#fff}.surfside-youversion-versions ul{columns:2;column-gap:28px;margin:0;padding-left:20px}.surfside-youversion-proof p{margin:0 0 8px}.surfside-youversion-proof small{display:block;color:#65758a;line-height:1.35}.surfside-youversion-proof a{display:inline-block;margin-top:7px;font-size:.88rem}@media(max-width:680px){.surfside-youversion-row{grid-template-columns:1fr auto auto auto}.surfside-youversion-row input{grid-column:1/-1}.surfside-youversion-meta{display:block}.surfside-youversion-meta label{display:block;margin-top:5px}.surfside-youversion-versions ul{columns:1}.surfside-youversion-diagnostic summary span{margin-left:0}.surfside-youversion-diagnostic summary{flex-wrap:wrap}}</style>
    <?php
    return ob_get_clean();
}
