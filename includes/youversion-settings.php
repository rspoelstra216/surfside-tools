...
function surfside_tools_youversion_settings_handle_post() {
    ...
    if ($action === 'test' || $action === 'passage_test') {
        if ($action === 'passage_test') {
            $passage = function_exists('surfside_tools_youversion_request')
                ? surfside_tools_youversion_request('bibles/3034/passages/JHN.3.16')
                : new WP_Error('surfside_youversion_client_missing', 'YouVersion client is unavailable.');
            $version = function_exists('surfside_tools_youversion_request')
                ? surfside_tools_youversion_request('bibles/3034')
                : new WP_Error('surfside_youversion_client_missing', 'YouVersion client is unavailable.');

            if (is_wp_error($passage)) {
                $test = array('success' => false, 'message' => surfside_tools_youversion_format_error($passage));
            } elseif (is_wp_error($version)) {
                $test = array('success' => false, 'message' => surfside_tools_youversion_format_error($version));
            } else {
                $test = array(
                    'success' => true,
            ...
            if (is_wp_error($result)) {
                $test = array('success' => false, 'message' => surfside_tools_youversion_format_error($result));
            } else {
    ...
    <section id="surfside-youversion" class="surfside-front-settings-card surfside-youversion-settings-card">
        ...
        <?php if (is_array($test)) : ?>
            <div class="surfside-front-settings-notice <?php echo !empty($test['success']) ? 'surfside-front-settings-success' : 'surfside-front-settings-error'; ?> surfside-youversion-notice">
                <?php echo esc_html($test['message'] ?? 'YouVersion test completed.'); ?>
                <?php if (!empty($test['success']) && !empty($test['count'])) : ?> Accessible Bible versions: <?php echo esc_html(number_format_i18n(absint($test['count']))); ?>.<?php endif; ?>
            </div>

            <?php if (!empty($test['proof']) && is_array($test['proof'])) : $proof = $test['proof']; ?>
                <div class="surfside-youversion-proof">
                    <strong><?php echo esc_html(($proof['reference'] ?? 'John 3:16') . ' · ' . ($proof['version'] ?? 'BSB')); ?></strong>
                    <p><?php echo esc_html($proof['content'] ?? ''); ?></p>
                    <?php if (!empty($proof['copyright'])) : ?><small><?php echo esc_html($proof['copyright']); ?></small><?php endif; ?>
                    <?php if (!empty($proof['deep_link'])) : ?><a href="<?php echo esc_url($proof['deep_link']); ?>" target="_blank" rel="noopener">Open version in YouVersion ↗</a><?php endif; ?>
                </div>
            <?php elseif (!empty($test['success']) && !empty($test['versions']) && is_array($test['versions'])) : ?>
        ...
        <form method="post" class="surfside-youversion-form">
            ...
            <div class="surfside-youversion-row">
                ...
                <?php if ($configured) : ?>
                    ...
                    <button type="submit" name="surfside_youversion_settings_action" value="passage_test" class="surfside-front-secondary-button surfside-youversion-button">Verse</button>
                <?php endif; ?>
            </div>
            <div class="surfside-youversion-meta">
                <span>Test checks licensed versions; Verse verifies John 3:16 text + attribution. The saved key is never displayed.</span>
                <?php if ($configured) : ?>
