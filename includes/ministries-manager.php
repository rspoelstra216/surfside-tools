...
function surfside_tools_staff_ministries_manager_shortcode() {
    ...
    <div class="surfside-staff-shell surfside-information-manager">
      <div class="surfside-staff-back"><a href="<?php echo esc_url(surfside_tools_staff_page_url('site-management')); ?>">← Back to Manage Website</a></div>
      <section class="surfside-staff-hero"><p class="surfside-staff-eyebrow">Website &amp; App</p><h1>Ministries</h1><p class="surfside-staff-muted">Manage ongoing ministries and choose who each ministry is for. Bible studies are managed separately through Calendar Manager.</p></section>
      <?php echo $notice; ?>
      <form method="post" class="surfside-information-form">
        ...
        <div class="surfside-information-ministries" data-surfside-ministries>
          ...
          <section class="surfside-information-card surfside-information-ministry">
            ...
            <fieldset class="surfside-ministry-audiences"><legend>Audience</legend><?php foreach($audiences as $key=>$label): ?><label class="surfside-information-checkbox"><input type="checkbox" name="ministries[<?php echo esc_attr($index); ?>][audiences][]" value="<?php echo esc_attr($key); ?>" <?php checked(in_array($key,(array)($ministry['audiences'] ?? array('adults')),true)); ?>> <?php echo esc_html($label); ?></label><?php endforeach; ?></fieldset>
        ...
        <template data-ministry-template><section class="surfside-information-card surfside-information-ministry"><input type="hidden" name="ministries[__INDEX__][key]" value=""><label class="surfside-information-field"><span>Icon</span><input type="text" name="ministries[__INDEX__][icon]" maxlength="12" placeholder="🙏"></label><label class="surfside-information-field"><span>Ministry name</span><input type="text" name="ministries[__INDEX__][name]" required></label><label class="surfside-information-field"><span>Usual schedule</span><input type="text" name="ministries[__INDEX__][schedule]"></label><label class="surfside-information-field"><span>Usual location</span><input type="text" name="ministries[__INDEX__][location]"></label><fieldset class="surfside-ministry-audiences"><legend>Audience</legend><?php foreach($audiences as $key=>$label): ?><label class="surfside-information-checkbox"><input type="checkbox" name="ministries[__INDEX__][audiences][]" value="<?php echo esc_attr($key); ?>" <?php checked($key,'adults'); ?>> <?php echo esc_html($label); ?></label><?php endforeach; ?></fieldset><label class="surfside-information-field surfside-information-ministry-description"><span>Description</span><textarea name="ministries[__INDEX__][description]" rows="3"></textarea></label><div class="surfside-information-ministry-actions"><button type="button" class="surfside-information-remove" data-ministry-up>↑</button><button type="button" class="surfside-information-remove" data-ministry-down>↓</button><button type="button" class="surfside-information-remove" data-ministry-remove>Remove</button></div></section></template>
